...
public class LastMapper extends Mapper<LongWritable,Text,Text,Text> {
    public static Map<String,Integer> cmap = null;//<count,N>
    public static Map<String,Integer> df = null;//《分词，DF》


    protected void setup(Context context) throws IOException, InterruptedException {
        if (cmap == null || cmap.size() == 0 || df == null || df.size() == 0){
            URI[] ss = context.getCacheFiles();//获取缓存文件URI
            if (ss != null){
                for (int i = 0; i <ss.length ; i++) {
                    ...
                    if (uri.getPath().endsWith("part-r-00003")){
                        ...
                        while ((line = br.readLine()) != null){
                            String[] ls =line.split("\t");
                            df.put(ls[0],Integer.parseInt(ls[1]));
                        }
                        br.close();
                    }
                }
